...
use Madfoat\MadfoatPayments\Helper\MadfoatPayments as MadfoatPaymentsHelper;
...
class MadfoatPayments extends \Magento\Payment\Model\Method\AbstractMethod {
    const CODE = 'madfoat_madfoatpayments';
    ...
    protected $_formBlockType = 'Madfoat\MadfoatPayments\Block\Form\MadfoatPayments';
    protected $_infoBlockType = 'Madfoat\MadfoatPayments\Block\Info\MadfoatPayments';
    ...
    public function __construct(
        ...
        \Madfoat\MadfoatPayments\Helper\MadfoatPayments $helper,
    ...
    public function refund(\Magento\Payment\Model\InfoInterface $payment, $amount)
    {
        ...
        if ($remoteApiKey == "") {
            throw new \Magento\Framework\Exception\LocalizedException(__('Please configure Madfoat Remote API Key to initiate the refund.'));
        }
        ...
        if($parsedResponse['auth_status'] != 'A'){
            throw new \Magento\Framework\Exception\LocalizedException(__('The refund with Madfoat is failed with message: ' . $parsedResponse['auth_message']));
        }else{
        	$payment
    ...
    public function capture($payment, $amount)
    {
        ...
        if ($remoteApiKey == "") {
            throw new \Magento\Framework\Exception\LocalizedException(__('Please configure Madfoat Remote API Key to capture payments online.'));
        }

        if(!isset($payment->getAdditionalInformation()['madfoat_ref'])){
        ...
        $data =array(
            ...
            'ivp_amount' => $amount,
            'ivp_test' => $this->getConfigData('sandbox') ? 1 : 0,
            'tran_ref'     => $payment->getAdditionalInformation()['madfoat_ref'] 
        );
        ...
        if($parsedResponse['auth_status'] != 'A'){
            throw new \Magento\Framework\Exception\LocalizedException(__('The Capture Request with Madfoat is failed with message: ' . $parsedResponse['auth_message']));
        }
    ...
    public function void($payment)
    {
        ...
        $this->paymentVoided($payment->getAdditionalInformation()['madfoat_ref'] , $payment->getOrder()->getOrderCurrencyCode(), $payment->getOrder()->getGrandTotal());
    ...
    {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, 'https://uat-secure.madfoat.dev/gateway/remote.html');
     ...
     */
    public function buildMadfoatRequest($order) {
        ...
        $madfoat_lang = $this->getConfigData('madfoat_lang');

        $txnType = $this->getConfigData("madfoat_txn_type");
        ...
        try {
            ...
            if (isset($results['order']['ref']) && isset($results['order']['url'])) {
                ...
                $paymentToken['madfoat_order_ref'] = $ref;
                ...
                if(isset($results['error'])){
                    $_SESSION['madfoat_error_message'] = $results['error']['message'] . ": " . $results['error']['note'];
    ...
    private function registerAuth($message,$txref) {
        ...
        $payment->setAdditionalInformation('madfoat_message', $message);
        $payment->setAdditionalInformation('madfoat_ref', $txref);
        $payment->setAdditionalInformation('madfoat_status', \Magento\Sales\Model\Order\Payment\Transaction::TYPE_AUTH);
        $payment->place();
        ...
        try {
            ...
            $message = __('The authorized amount in Madfoat is %1.', $formatedPrice);
    ...
    private function registerPending($message,$txref) {
        ...
        $payment->setAdditionalInformation('madfoat_message', $message);
        $payment->setAdditionalInformation('madfoat_ref', $txref);
        $payment->setAdditionalInformation('madfoat_status', 'Pending');
        $payment->place();
    ...
    private function registerCapture($message,$txref) {
        ...
        $payment->setAdditionalInformation('madfoat_message', $message);
        $payment->setAdditionalInformation('madfoat_ref', $txref);
        $payment->setAdditionalInformation('madfoat_status', \Magento\Sales\Model\Order\Payment\Transaction::TYPE_CAPTURE);
        $payment->place();
    ...
    private function paymentCompleted($txref) {
        ...
        $message='Payment completed by Madfoat: '.$txref;
    ...
    private function paymentPending($txref) {
        ...
        $message='Payment pending by Madfoat: '.$txref;
    ...
    private function paymentAuthorised($txref) {
        ...
        $message='Payment authorisation by Madfoat: '.$txref;
        $state=$this->getStateCode("processing");
        $this->updateOrder($message, $state, $state, false);
    }
    ...
    private function paymentCancelled() {
        $message='Payment request cancelled by Madfoat';
    ...
    public function validateResponse($order_id) {
        ...
        //$madfoat_order_ref = $this->getCheckoutSession()->getOrderRef();
        ...
        $madfoat_order_ref = $addonInfo['madfoat_order_ref'];
            ...
            if ($txStatus=='A') {
                ...
                if(isset($paymentToken['save_card']) && $paymentToken['save_card'] == 'yes' && isset($results['order']['card']) && !$this->_order->getCustomerIsGuest()){
                    try{
                        ...
                        $newToken->setPaymentMethodCode('madfoat_madfoatpayments');
    ...
    public function getIframeUrl() {
        $url = $this->helper->getUrl('madfoat/standard/iframe')  . "?tx=" . time();
        return $url;
    }
    ...
    public function updateOrderStatusWithMessage($order, $status, $txnref, $addTxn = false){
        ...
        switch ($status) {
            case 'complete':
                $message='Payment completed by Madfoat: ' . $txnref;
                $state=$this->getStateCode("processing");
                break;

            case 'canceled':
                $message='Payment request canceled by Madfoat: ' . $txnref;
                $state=$this->getStateCode("canceled");
                break;

            case 'refunded':
                $message='Transaction Refunded by Madfoat: ' . $txnref;
                $state=$this->getStateCode("closed");
                break;
            

            case 'processing':
                $message='Madfoat Transaction Reference: ' . $txnref;
                $state=$this->getStateCode("processing");
                break;
            ...
            default:
                $message = 'Transaction ' . $status . ' by Madfoat: ' . $txnref;
